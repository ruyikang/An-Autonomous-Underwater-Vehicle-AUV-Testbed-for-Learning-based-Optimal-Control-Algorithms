...
import joblib
model = joblib.load(r"model_obs2.pkl")
...
# width and centimeters
    ...
    height_blue = [195,187.421,182.789,173.5263,155]
    height_black = [149,137.421,132.789,123.5263,105]
    #width1 = [50,41]
    x_pixel_sample = [110,171,195,243.2,340]
    y_pixel_sample = [20,62,79,113,180]
    conver_x_sample = [15.27778,15.88889,16.17857,13.57193,9.971831]
    conver_y_sample = [16.77273,14.31579,13.22727,12.09091,8.47619]
    depth = [48,53,55,59,67] # y=Ax+B  OR y = Ax^2+bx+C
    #depth1 = [48,67]
    coff_xpixel = np.polyfit(depth,x_pixel_sample,3)
    equ1 = np.poly1d(coff_xpixel)
    coff_ypixel = np.polyfit(depth,y_pixel_sample,3)
    equ2 = np.poly1d(coff_ypixel)
    coff_xconver = np.polyfit(depth,conver_x_sample,3)
    equ3 = np.poly1d(coff_xconver)
    coff_yconver = np.polyfit(depth,conver_y_sample,3)
    equ4 = np.poly1d(coff_yconver)
    coff_width = np.polyfit(width, depth, 3)
    equ5 = np.poly1d(coff_width)
    coff_heightblue = np.polyfit(height_blue, depth, 3)
    equ6 = np.poly1d(coff_heightblue)
    coff_heightblack = np.polyfit(height_black, depth, 3)
    equ7 = np.poly1d(coff_heightblack)
    while isOpened:
        counter = counter + 1
        flag, img = cap.read()  # read every frame
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        #pixel = int(np.mean(gray[gray > 140]))
        #gray[gray > 50] = pixel
        gray1 = gray
        gray1[gray1 > 200] = 255
        gray1[gray1 < 50] = 0  # 把中间像素转化为黑与白
        blur = cv2.medianBlur(gray1, 3)  # median blur
        ret, dst = cv2.threshold(blur, 115, 255, cv2.THRESH_BINARY_INV) # 与环境有关
        ...
        AREA_THRESHOLD_L = 2000  #4000
        AREA_THRESHOLD_H = 200000  #20000
        canvas = np.copy(img)
        # top 4 points
        ...
        for i, cnt in enumerate(contours):
            ...
            if w * h < AREA_THRESHOLD_L:
                ...
                continue
            #elif w > 180 or h > 180:  #防止检测出长而宽的裂痕
            #    continue
            ...
            if int(width) > 300 or int(height) > 300: #防止检测出长而宽的裂痕
                continue
            ...
            if plate_type[res_index] == 'black':
                ...
                if abs(int(equ5(pref_short)) - distance) > 4:
                    distance = int(equ5(pref_short))
            if distance > topwater + 0.7:
                distance = distance * 1.1166 #水的折射 可调
            if distance < 48:
                distance = 48
            elif distance > 67:
                distance = 67
            ...
            center_xy_real = (center_x_real,center_y_real,yaw_actual)
            width_distance = (distance,pref_long)
            compare_wh = (int(equ5(pref_short)),int(equ6(pref_long)))
            #print(center_xy_real)
            print(distance,":",center_x,",",center_y,"(",x_pixel,y_pixel,")","(",conver_x,conver_y,")",center_x_real,",",center_y_real,",",yaw_actual)
            print(cX, cY, center)
            ...
            cv2.putText(canvas, str(width_distance), (20, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255),
                            2)
            ...
            mask_red = cv2.inRange(img_hsv, color_dict['red'][0], color_dict['red'][1])
            mask_black = cv2.inRange(img_hsv, color_dict['black'][0], color_dict['black'][1])
            blue_sum = (mask_blue == 255).sum()
            red_sum = (mask_red == 255).sum()
            black_sum = (mask_black == 255).sum()
            sum_list = np.array([blue_sum, red_sum,black_sum])
            res_index = np.argmax(sum_list)  # choose the maximum value
            plate_type = ["blue", "red", "black"]
            if plate_type[res_index] == 'blue':
                 record_bluedistance.append(distance)
            # elif plate_type[res_index] == 'red':
            #     print(timestamp, 'Submarine:red')
            ...
            if (cX > int(min_area_rect[0][0])) and (cY >= int(min_area_rect[0][1])): # 重心在右下，矩阵中心在左上
                #print("重心右下，中心左上")
        ...
        if counter%2 == 0:
            ...
            obs = [[-center_y_real+20, -center_x_real+30, yaw_actual2]]
