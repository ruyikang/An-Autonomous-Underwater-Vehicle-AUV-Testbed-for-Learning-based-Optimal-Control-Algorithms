...
def main():
    ...
    while isOpened:
        ...
        for i, cnt in enumerate(contours):
            ...
            cX = int(M["m10"] / M["m00"])
            cY = int(M["m01"] / M["m00"])
            # corners = cv2.goodFeaturesToTrack(obj_bin, maxCorners=1, qualityLevel=0.01, minDistance=10)
            #
            # # 将角点坐标从浮点数转换为整数
            ...
            #error adjustion
            if abs(cY-int(center_y)) <= 2:
                counter_para = 0
                state_para == 0
                if counter_para % 15 == 0:
                    state_para == 1

            if (cX > int(min_area_rect[0][0])) and state_para == 1:
                cY = cY + 2
            elif (cX < int(min_area_rect[0][0])) and state_para == 1:
                cY = cY - 2
